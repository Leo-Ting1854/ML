...
class PLA:
    ...
    def fit(self,X,y,pocket):
        ...
        for sort in kind:  #ex kind=[0,1,2]  then need 3 perceptron
            ...
            if(error_time>most_error_sort):#多個符合 必為卡在中間的模糊地帶
                most_error_sort=error_time
    ...
    def predict(self,X):
        ...
        for x_vector in X_vector:
            ...
            for W_index in range (len(W_vector)):#依序套入感知器
                ...
                last_W=W_vector[W_index]
            if(sort_done_flag==0):
                y_predict.append(np.random.randint(0,self.kind[len(self.kind)-1]))
            elif(sort_done_flag==1):
                y_predict.append(np.argmax(sort))
            else:#被重複分類的話 就使用錯最多的那個
                y_predict.append(self.most_error)
...
def main():
    ...
    # y=iris.target[0:100]
    #blobs
    #X,y=datasets.make_blobs(200,n_features=2)
    #digit
